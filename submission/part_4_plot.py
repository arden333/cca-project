...
def plot_everything(log_file, mc_file):
    ...
    for ev in vis_events:
        ...
        for cid in ev.cores:
            if not p and ev.etype == "pause":
                a = ax_alloc.scatter(
                    t_s,
                    cid + 0.45 if ev.etype == "pause" else cid - 0.45,
                ...
                p = [a]
            if len(p) == 1 and ev.etype == "unpause":
                a = ax_alloc.scatter(
                    t_s,
                    cid + 0.45 if ev.etype == "pause" else cid - 0.45,
                    marker="v" if ev.etype == "pause" else "^",
                ...
                p.append(a)
            ax_alloc.vlines(
                t_s, cid - 0.4, cid + 0.4, colors=color, linewidth=0.1, alpha=0.9
            )
            ax_alloc.scatter(
                t_s,
                cid + 0.45 if ev.etype == "pause" else cid - 0.45,
    ...
    if mc_metrics:
        ...
        ax1_left.legend(loc="upper left", bbox_to_anchor=(0.0, 0.95))
        ax1_right.legend(loc="upper right", bbox_to_anchor=(1.0, 0.95))
    ...
    plt.tight_layout()
    plt.savefig("plot4.3-run-1.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    plot_everything("./results-part4.3/jobs_1.txt", "./results-part4.3/mcperf_1.txt")
